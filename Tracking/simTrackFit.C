...
//     struct
//****************************************************************************************
// Usage example with 4p track fitting (#include this file in your main macro):
// 
...
// Float_t for residuals in stn2 --> myFitted_results.residuals[1]
// Float_t for residuals in stn3 --> myFitted_results.residuals[2]
// Float_t for residuals in stn4 --> myFitted_results.residuals[3]
//
//****************************************************************************************
// To be added:
...
fitResults TrackFit_4p(Float_t pointX[4], Float_t pointY[4]){
  ...
  Float_t errX[4] = {0.04,0.04,0.04,0.04};        //0.085
  Float_t errY[4] = {0.04,0.04,0.04,0.04};        //0.065  <-- these values best reproduce NERO chisq dists 
  ...
  */
  ...
  if(abs(y2Exp - pointY[1]) < limit && abs(y3Exp - pointY[2]) < limit){
    ...
    //uncomment the line below to see the hits in plot
    //linGrph->Draw();
...
//=========================================================================
// fitting function for 3 points data that outputs "fitResults", i.e.,
// a boolean for valid tracks and an array with fit results and residuals
//=========================================================================
fitResults TrackFit_3p(Float_t pointX[3], Float_t pointY[3]){
  ...
  Float_t resP1=0., resP2=0., resP3=0., resP4=0.;
  //uncertainties in nth module position and cluster position both set to 10mm here
  Float_t errX[3] = {0.04,0.04,0.04};        //0.085
  Float_t errY[3] = {0.04,0.04,0.04};        //0.065  <-- these values best reproduce NERO chisq dists 

  //===============================
  //  cluster alignment test
  //===============================
  Float_t slope = (pointY[2]-pointY[0])/(pointX[2]-pointX[0]);
  Float_t intercept = pointY[0] - (pointX[0]*slope);
  Float_t y2Exp = (slope*pointX[1]) + intercept;
  ...
  if(abs(y2Exp - pointY[1]) < limit){
    ...
    // setting up the fitting function
    TF1 *linFit_func = new TF1("linFit_func","pol1",-1.,2.);
    TGraphErrors *linGrph = new TGraphErrors(3,pointX,pointY,errX,errY); 
    linGrph->Fit("linFit_func","RQS");
    //uncomment the line below to see the hits in plot
    //linGrph->Draw();
    ...
    //==== some more checks for debugging =================================
    //cout << "=====================================================" << endl;
    //cout << "Inside the TrackFit_3p fuction"  << endl;
    //cout << "=====================================================" << endl;
    //cout << "Boolean          :  " << isTrk << endl;
    //cout << "Fitted Intercept :  " << fitIntercept << endl;
    //cout << "Fitted Slope     :  " << fitSlope << endl;
  ...
  result.residuals[1] = resP2;
  result.residuals[2] = resP3;
  result.residuals[3] = resP4;      //Note: this one is always zero for 3p tracking
